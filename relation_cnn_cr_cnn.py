...
def train(argv=None):
    ...
    init_m = math.sqrt(6.0 / (max_document_length + NUM_CLASSES - 1))
    M = tf.Variable(tf.random_uniform([max_document_length, NUM_CLASSES - 1],
                                                minval=-init_m,
                                                maxval=init_m,
                                                dtype=tf.float32))

    # class embeddings matrix
    init_class = math.sqrt(6.0 / (d_c + NUM_CLASSES - 1))
    classes_matrix = tf.Variable(tf.random_uniform([d_c, NUM_CLASSES - 1],
                                                minval=-init_class,
                                                maxval=init_class,
                                                dtype=tf.float32))
    ...
    def model(data,pf,sdp):
        ...
        # A_P = [batch_size , n, num_class - 1]
        A_P = tf.reshape(tf.matmul(H, classes_matrix), [-1, max_document_length, NUM_CLASSES - 1])
        # alpha = [batch_size, 1, num_class - 1]
    ...
    def get_targe_neg(size,labels,predicts):
        ...
        true_idx_flattened = tf.range(0,current_batch_size) * (NUM_CLASSES -1) + true_index_other
        ...
        # flag, other = 0, non-other = 1
        return true_values,neg_values, tf.cast(other_flag, tf.float32)
    ...
    t_values, n_values, flag_values = tf.cond(tf.equal(dropout_keep_prob,is_train),lambda: get_targe_neg(BATCH_SIZE,train_label,logits),
                                 lambda: get_targe_neg(Test_Size,train_label,logits))
    # pairwise ranking loss
    true_part = tf.log(1 + tf.exp(2*(2.5 - t_values)))
    false_part = tf.log(1 + tf.exp(2*(0.5 + n_values)))
    loss = tf.reduce_mean(true_part*flag_values + false_part)
    ...
    # Evaluate model
    # expand logits to contain other
    exp_zeros = tf.cond(tf.equal(dropout_keep_prob,is_train),lambda: tf.zeros(shape=[BATCH_SIZE, 1], dtype=tf.float32),
                        lambda: tf.zeros(shape=[Test_Size, 1], dtype=tf.float32))
    # exp_logits = [batch_size,num_class]
    exp_logits = tf.concat(1, [logits, exp_zeros])
    train_predict = tf.argmax(exp_logits,1)
    train_label = tf.argmax(train_labels_node,1)
    # train accuracy
