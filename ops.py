...
def conv2d(x, filter_num, filter_size,filter_length,stddev=0.1, name="conv2d"):
    with tf.variable_scope(name):
        w = tf.get_variable('w', [filter_size, filter_length, 1, filter_num],initializer=tf.contrib.layers.xavier_initializer())
        b = tf.get_variable('b', [filter_num], initializer=tf.zeros_initializer)
        conv = tf.nn.conv2d(x, w, strides=[1, 1, 1, 1], padding='VALID')
        tanh = tf.tanh(tf.nn.bias_add(conv, b))
        return tanh
...
def batch_norm_conv2d(x, filter_num, filter_size,idx,filter_length,is_training,stddev=0.1, name="conv2d"):
    with tf.variable_scope(name):
        ...
        # add batch norm
        conv = batch_norm_layer(conv, is_training, scope="kernel%d_batch_norm" % idx)
        tanh = tf.tanh(conv)
        return tanh
...
def batch_norm_layer(inputs, phase_train, scope=None):
    return tf.cond(phase_train,
                   lambda: tf.contrib.layers.python.layers.batch_norm(inputs, is_training=True,
                                                                      center=True, scale=True, activation_fn=None, decay=0.9, scope=scope),
                   lambda: tf.contrib.layers.python.layers.batch_norm(inputs, is_training=False,
                                                                      center=True, scale=True, activation_fn=None, decay=0.9, scope=scope, reuse=True))
